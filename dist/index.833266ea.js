...
})({"kXs6m":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
...
function makeFeed(feeds) {
    for(let i = 0; i < feeds.lengh; i++)feeds[i].read = false;
    return feeds;
}
//이벤트 hashchange 
function newsDetail() {
  ...
  `;
    for(let i = 0; i < store.feeds.length; i++)//const id = location.hash.substr(7)
...
function newsFeed() {
  ...
  `;
    //의미 no
    // newsList.push('<ul>');
    if (newsFeed.length === 0) newsFeed = store.feeds = makeFeed(getData(NEWS_URL));
    for(let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++)//태그 create먼저
    ...
    newsList.push(`<div class="p-6 ${newsFeed[i].read ? 'bg-red-500' : 'bg-white'} mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100"> 
        <div class="flex">
          <div class="flex-auto">
...
},{}]},["kXs6m","lzYRN"], "lzYRN", "parcelRequire94c2")
